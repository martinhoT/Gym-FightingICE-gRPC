...
class FightingiceEnv_TwoPlayer(gym.Env):
    ...
    def __init__(self, freq_restart_java=3, env_config=None, java_env_path=None, port=None, auto_start_up=False, frameskip=False, display=False, p2_server = None):
        ...
        start_system_lib_path = os.path.join(
            "lib", "lwjgl", "natives", system_name, self.system_type)
    ...
    def _start_java_game(self):
        ...
        if self.need_set_memory_when_start:
            ...
            self.java_env = subprocess.Popen(["java", "-cp", self.start_up_str, "Main", "--port", str(self.port), "--py4j", "--fastmode",
                                            "--grey-bg", "--inverted-player", "1", "--mute", "--limithp", "400", "400"], cwd=self.java_env_path, stdout=devnull, stderr=devnull)        # self.java_env = subprocess.Popen(["java", "-cp", "/home/myt/gym-fightingice/gym_fightingice/FightingICE.jar:/home/myt/gym-fightingice/gym_fightingice/lib/lwjgl/*:/home/myt/gym-fightingice/gym_fightingice/lib/natives/linux/*:/home/myt/gym-fightingice/gym_fightingice/lib/*", "Main", "--port", str(self.free_port), "--py4j", "--c1", "ZEN", "--c2", "ZEN","--fastmode", "--grey-bg", "--inverted-player", "1", "--mute"])
    ...
    def reset(self, options):
        p1 = options.get("p1", GymAI)
        p2 = options.get("p2", GymAI)
